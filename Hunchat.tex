...
Hunchat is a Lisbon based startup who wants to connect people online in a more meaningful way. The tools most people are using to communicate is not best fit for the types of conversation we should be having: meaningful ones. They believe the way to do it is by using video it's more organic - after all is the most similar to real world interaction - it's more complete - you never need emojis because you can do the actual expressions - and it's honest - less misunderstandings due to 'not getting the tone' - and intuitive - you just open the app and do it, you don't have to write or schedule a call, it's instant. So they created Hunchat an app that does exactly that connect people with video in an asynchronous way. By organising the content in threads, just as one might see on reddit or twitter, they created the perfect space for meaningful discussions online. Now that that's done, one problem surges: how are they going to get it used by actual people?

That is where an Integrated Marketing Communication Plan, or IMC for short comes in. An IMC is a structured way to plan all the communication for an organisation guaranteeing that the message stands consistent on all different media. The impact it can have of the performance of a business is immeasurable. Since Hunchat hasn't yet done any structured planing of communication in such an embryonic stage of their existence makes them the perfect company to expand a plan on. The main goal of the plan is of course get people using the app, and for that there's the need to communicate exactly what the product is in a clear way. Added to that the brand needs more recognition and so increasing both brand awareness and brand notoriety is the third goal.

The dissertation is structured in three parts: (1) an introductory literature review on the history of the and importance of the IMC plan, current state of social media and how an IMC could really help; (2) a method section where I go trough the different ways the data was collected and briefly reveal its results; and finally (3) the actual IMC plan for Hunchat where I go trough a analysis of the context and description of the areas and tactics to apply, and how to apply them.
...
Since Hunchat does not yet have a structures way of communicating,
